...
The dataset exhibits `r data$data_prepare$number_of_features` features, of which `r length(data$data_prepare$factor_attributes)` were handled as categorical, of which `r length(data$data_prepare$ordered_attributes)` are ordered.
...
feature_visualizer$visualizeCosmos(workspace_dir = file.path(install_dir, "workspace") , meta_instance = data$metafeatures)
```
